...
{
    [CustomPropertyDrawer(typeof(PUPrice))]
    public class PUPricePropertyDrawer : UnityEditor.PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            SerializedProperty priceProperty = property.FindPropertyRelative("amount");

            EditorGUI.PrefixLabel(new Rect(x, y, width, height), new GUIContent(property.displayName));
            EditorGUI.PropertyField(new Rect(x + offset, y, width, height), property.FindPropertyRelative("powerUpType"), GUIContent.none);
