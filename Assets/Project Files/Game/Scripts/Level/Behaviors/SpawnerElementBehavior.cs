...
namespace Watermelon.BusStop
{
    public class SpawnerElementBehavior : LevelElementBehavior
    {
        ...
        public ElementPosition SpawnerOffset => spawnerOffset;
        ...
        public bool IsSpawnSlotOccupied()
        {
            return LevelController.GetElementType(spawnerOffset) != LevelElement.Type.Empty;
        }

        public void ReturnToSpawner()
        {
            LevelElement.Type levelElement = LevelController.GetElementType(spawnerOffset);

            ReturnToSpawner(levelElement);
        }

        public void ReturnToSpawner(LevelElement.Type levelElement)
        {
            if (levelElement != LevelElement.Type.Empty)
            {
                spawnQueue.Add(levelElement);

                currentElementIndex++;

                amountText.text = currentElementIndex.ToString();

                LevelController.RemoveLevelObject(spawnerOffset, false);
            }
        }
