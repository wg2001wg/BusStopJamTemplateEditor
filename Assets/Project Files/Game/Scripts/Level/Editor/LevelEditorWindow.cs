...
namespace Watermelon.BusStop
{
    public class LevelEditorWindow : LevelEditorBase
    {
        ...
        private const string EDITOR_SPAWNER_PROPERTY_NAME = "editorSpawnerTexure";
        ...
        private float currentQueueWidth;
        ...
        protected override void ReadLevelDatabaseFields()
        {
            ...
            editorSpawnerTexureSerializedProperty = levelsDatabaseSerializedObject.FindProperty(EDITOR_SPAWNER_PROPERTY_NAME);
        }
        ...
        protected override void InitializeVariables()
        {
            ...
            tabHandler.AddTab(new TabHandler.Tab("Settings", DisplayProperties));
        ...
        private Color GetColor(int index, LevelElement.Type typeName)
        {
            ...
            for (int i = 0; i < levelElementsSerializedProperty.arraySize; i++)
            {
                elementProperty = levelElementsSerializedProperty.GetArrayElementAtIndex(i);
            ...
            levelElementsSerializedProperty.arraySize++;
            elementProperty = levelElementsSerializedProperty.GetArrayElementAtIndex(levelElementsSerializedProperty.arraySize - 1);
        ...
        private void DrawBusSpawnQueue()
        {
            ...
            queueDrawRect = GUILayoutUtility.GetRect(1, Screen.width, 0, 0, GUILayout.ExpandWidth(true));

            if ((queueDrawRect.width > 1) && (Event.current.type == EventType.Repaint))
            {
                currentQueueWidth = queueDrawRect.width;
            }
            ...
            itemsPerRow = Mathf.FloorToInt((currentQueueWidth - 16) / (queueElementSize));
        ...
        private void DrawLevel()
        {
            ...
            for (int x = 0; x < selectedLevelRepresentation.widthProperty.intValue + 1; x++)
            {
                lineRect.Set(drawRect.x + (x * elementSize), drawRect.y, 2, selectedLevelRepresentation.heightProperty.intValue * elementSize);
                DrawColorRect(lineRect, Color.gray);
            }

            for (int y = 0; y < selectedLevelRepresentation.heightProperty.intValue + 1; y++)
            {
                lineRect.Set(drawRect.x, drawRect.y + (y * elementSize), selectedLevelRepresentation.widthProperty.intValue * elementSize, 2);
                DrawColorRect(lineRect, Color.gray);
            }
