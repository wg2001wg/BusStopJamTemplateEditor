...
namespace Watermelon.BusStop
{
    ...
    public class DockBehavior : MonoBehaviour
    {
        ...
        private SlotCase addedSlotCase = null;
        ...
        private void RemoveMatch(List<BaseCharacterBehavior> charactersToRemove)
        {
            ...
            Tween.DoFloat(1f, 1.3f, 0.3f, (value) => {
                ...
                if (addedDepth > 0 && !slots[^3].IsOccupied)
                {
                    addedDepth--;
                    for (int i = 0; i < 3; i++)
                    {
        ...
        public bool SubmitToSlot(BaseCharacterBehavior element, bool instant)
        {
            ...
            if (CheckMatch(false))
            {
                if (addedDepth < 2)
                {
                    addedSlotCase = slotCase;
                    AddTempSlots();
                }
            }
            else if ((GameController.Data.ActivateVehicles && IsFilled && ShouldWaitForBus()))
            {
                if (addedDepth < 2)
                {
                    addedSlotCase = slotCase;
        ...
        {
            LevelElement.Type busType;
            if (EnvironmentBehavior.CollectingBus.PassengersCount != 3)
            {
                busType = EnvironmentBehavior.CollectingBus.Type;
            }
            else
            {
                busType = EnvironmentBehavior.WaitingBus.Type;
            }

            for (int i = 0; i < slots.Count; i++)
            {
                var slot = slots[i];
                if (slot.SlotCase != null && slot.SlotCase.Behavior != null)
                {
                    if (slot.SlotCase.Behavior.LevelElement.ElementType == busType)
                    {
                        return true;
                    }
                }
        ...
        public void OnMoveEnded(SlotCase slotCase, MoveType moveType)
        {
            ...
            if (addedDepth > 0 && addedSlotCase != slotCase && !slots[^3].IsOccupied)
            {
                for (int i = 0; i < 3; i++)
                {
                    var slot = slots[slots.Count - 1];
